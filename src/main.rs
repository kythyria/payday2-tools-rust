...
fn main() {
    ...
    if argv.len() != 3 {
        //panic!("Specify filenames");
        for line in &argv {
            println!("{:x} {}", diesel_hash::hash(line.as_bytes()), line);
        }
        return;
    }
    let filename = &argv[1];
    ...
    let destfile = fs::File::create(&argv[2]).unwrap();
    let mut dest = std::io::BufWriter::new(destfile);

    for line in (&contents).lines() {
        let r = writeln!(dest, "{:x} {:?}", diesel_hash::hash(line.as_bytes()), line.as_bytes());
        r.unwrap();
    }
    dest.flush().unwrap();
}
